...
class ClientConfig:
    ...
    manifest_execution_profile: Optional[Dict[str, Any]] = None  # exact signed runtime profile from the manifest
    revocation_files: Sequence[str] = ()  # signed identity rotation/revocation bundles enforced before routing
    ...
    replay_chunk_size: int = 64  # max batch_size * tokens sent per RPC while rebuilding a failed attention cache
