...
from drift.node.linux_anchor_state import _hex, _integer
from drift.node.resource_recovery import RecoverableStateError


def request(operation, nonce, revision=None, request_id=None):
    ...
    if operation == "observe":
        anchor._require(revision is None and request_id is None)
    else:
        anchor._require(_integer(revision) and _hex(request_id))
    return dict(
        version=2, profile=anchor.PROFILE, operation=operation, nonce=nonce, revision=revision, request_id=request_id
    )
...
def respond(layout, controller, value):
    ...
    expected = request(value.get("operation"), value.get("nonce"), value.get("revision"), value.get("request_id"))
    anchor._require(anchor._encode(value) == anchor._encode(expected))
    layout.validate()
    if expected["operation"] != "observe":
        controller.submit(expected["operation"], expected["revision"], expected["request_id"])
    result = receipt(layout.service, layout.profiles, expected["operation"], expected["nonce"], controller.snapshot())
    layout.validate()
    return result


def control_anchor(operation="observe", *, revision=None, request_id=None):
    """Peer/service/layout-bound cached completion, never a maintenance lease."""
    try:
        nonce = os.urandom(32).hex()
        command = request(operation, nonce, revision, request_id)
