...
from communityai_anchor.resource_recovery import (
...
from communityai_anchor.resource_recovery_config import normalize_worker_cgroup_root
...
def observe_cgroup_profile(rootpath, *, require_unfrozen=True):
    """Check native identity/delegation without loading the node birth runtime.

    This grants no process-creation capability. Desktop observers need the same
    exact filesystem/topology checks, but never execute a process birth probe.
    """
    return _check_cgroup_profile(rootpath, require_unfrozen=require_unfrozen, require_backend=False)


def validate_cgroup_profile(rootpath, *, require_unfrozen=True):
    """Execution admission additionally requires the native birth capability."""
    return _check_cgroup_profile(rootpath, require_unfrozen=require_unfrozen, require_backend=True)


def _check_cgroup_profile(rootpath, *, require_unfrozen, require_backend):
    descriptor = None
    try:
        _platform()
