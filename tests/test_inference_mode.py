...
def test_control_mode_change_is_authenticated_persistent_and_preserves_active_answer(tmp_path):
    ...
    manager.register(ModelDescriptor("local", execution="local"), lambda: ModelRuntime(object(), None), route_health=lambda: local)
    ...
    app = create_node_app(manager, api_keys=["client-secret"], control_keys=["control-secret"],
                          worker_supervisor=supervisor, contribution_policy_store=store)
    with TestClient(app) as client:
        active = manager.load("auto")
        prior = path.read_bytes()
        body = {"inference_mode": "local_only", "expected_config_revision": store.snapshot()["config_revision"]}
        assert client.put("/control/v1/inference-mode", json=body,
                          headers={"Authorization": "Bearer client-secret"}).status_code == 401
        ...
        active.release()
        assert client.post("/v1/completions", json={"model": "community", "prompt": "hello"},
                           headers={"Authorization": "Bearer client-secret"}).status_code == 503
