...
def test_control_commands_have_fixed_bounded_contract():
    ...
    value = control.request("start", "b" * 64, 1, "c" * 32)
    assert value == dict(
        version=2, profile="multigpu-volunteer", operation="start", nonce="b" * 64, revision=1, request_id="c" * 32
    )
...
def test_invalid_commands_never_grant_launch(operation, revision, request_id):
    with pytest.raises(RecoverableStateError):
        control.request(operation, "b" * 64, revision, request_id)
