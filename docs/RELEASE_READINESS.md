...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Qwen image](evidence/gate4-20260826-b-qwen3.5-2b-publication-evidence.json) with its evidence-bound 9 GB rootfs requirement | Revalidate and reserve the authorized exact four-host GCP plan, then run all four profiles on distinct claimed machines |
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Gemma image](evidence/gate4-20260826-b-gemma-4-e2b-publication-evidence.json) with its evidence-bound 13 GB rootfs requirement | Reuse the authorized exact four-host GCP fleet after Gate 5 and retain all four distinct-profile results plus the aggregate |
