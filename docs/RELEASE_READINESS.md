...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | IN PROGRESS | [Attempts 1–3](evidence/qual-20260826-b-capacity-attempt-3.json) tried three N1/T4 primary regions, each hit `ZONE_RESOURCE_POOL_EXHAUSTED` on Windows CUDA, and fully cleaned every exact run resource while retaining the protected bootstrap. The same USD 69 reservation now has a [provider-authorized attempt-4 plan](evidence/qual-20260826-b-cost-plan-attempt-4.json) using preflighted G2/L4 CUDA hosts in `us-central1-b`/`us-east1-b`, exact images, `pd-balanced` CUDA disks, a 13.5-hour hard deadline, and CUDA-first creation. | Execute attempt 4 in order, verify exact boot sources and host prerequisites, then register and run all four distinct Qwen profiles against the immutable Gate 4 snapshot |
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | WAITING | The strict four-profile matrix can use the passed [immutable Gate 4 Gemma image](evidence/gate4-20260826-b-gemma-4-e2b-publication-evidence.json), but shared-fleet [attempts 1–3](evidence/qual-20260826-b-capacity-attempt-3.json) were cleaned before any qualification job and Gate 5 has not passed | Reuse the exact replacement fleet only after reviewing Gate 5, then retain all four Gemma profile results plus the aggregate |
...
| qual-20260826-b | GCP | Four-host Windows/Linux qualification fleet [source 7660e33e03326e5b868f81cb95282460ba649d5f] | USD 69.00 | — | [Attempts 1–3](evidence/qual-20260826-b-capacity-attempt-3.json) fully cleaned; attempt 4 not provisioned | PLANNED |
