...
| 7 | Qwen3.5 2B real separate-machine recovery | READY | Provider-neutral controller and the 45-test Fly suite pass locally; provisioning now requires an exact Qwen publication report, derives its immutable runtime manifest, and sends the measured bounded rootfs size on all five Machine creates before selected-worker hard kill and fail-closed cleanup | After its accepted Windows/Linux matrix and Gate 4 report exist, reserve a priced Fly ledger row, run one bootstrap plus four workers, kill the selected worker during generation, prove exact recovery and cleanup |
