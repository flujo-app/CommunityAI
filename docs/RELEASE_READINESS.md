...
| 16 | Complete public-alpha safety and operations | IN PROGRESS | The current repository slice gives manifested workers one shared finite global/per-PeerID active/rate authority, bounded hashed identity/session state, generation-safe aggregate push routing, finite manager-lock/shutdown behavior, aggregate-only health, training-off defaults, and an explicit disable/rollback runbook; 58 focused tests and the 504-pass/7-skip Windows/Linux offline selection pass | Run a bounded malicious-load canary against Hivemind connection/task/log-volume behavior, retain privacy-safe health reconstruction, execute the monitored limited rollout plus disable/rollback drill, and link immutable hosted evidence; no public rollout has run |
