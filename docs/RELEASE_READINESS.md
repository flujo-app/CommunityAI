...
| V | Pass a visible public vertical slice: app observes a remote worker, `auto` selects a model, and inference succeeds | BLOCKED | [Commit `2c49d68`](https://github.com/flujo-app/CommunityAI/commit/2c49d68892748650f1945fb9b21d3bb5371a8ae2) preserves signed-catalog priority, resolves `model: "auto"` only against a complete authenticated live DHT route, exposes the reason and exact coverage to the desktop, and fails unavailable with HTTP 503. [Follow-up `39e18d5`](https://github.com/flujo-app/CommunityAI/commit/39e18d55fd387a8a6723b1af23595b0baf73ab84) corrected the seven import groups exposed by the first PR run; [Black and isort](https://github.com/flujo-app/CommunityAI/actions/runs/33046732657), the supported [Ubuntu test job](https://github.com/flujo-app/CommunityAI/actions/runs/33046732760/job/98432374670), 102 focused local tests, independent tester review, and the exact-head [Windows/Linux production packages](https://github.com/flujo-app/CommunityAI/actions/runs/33046732723) pass. This remains implementation evidence only: the release catalog is not bundled and no public candidate worker or remote inference result exists. The [billing-reconciliation attempt](evidence/gate-v-20260827-billing-reconciliation-attempt.json) found no native report or billing-export dataset and created no resource; USD 99 therefore remains unreconciled, while the one-hour G2/L4 compute-and-disk floor already exceeds the USD 1 remainder before egress contingency. | Obtain observed cost from the Cloud Billing report or an export and replace the delayed maximums only when supported by that evidence. If the resulting remainder fits the full conservative estimate, bundle the release catalog, create one bounded Linux G2/L4 Qwen worker, join it to the public DHT, show all 24 blocks and the `auto` reason, generate through localhost, retain immutable evidence, and clean the VM. Do not provision under the current ledger, pull Gemma, or claim the gate from local tests. |
