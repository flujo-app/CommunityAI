...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Qwen image](evidence/gate4-20260826-b-qwen3.5-2b-publication-evidence.json). Provider preflight found one T4 slot in each of `us-central1` and `us-east1`; the 25-test plan now binds exact OS images and safely assigns only Linux CUDA to the fallback region without changing the USD 69 maximum. | Rerun the exact guard and require `provisioning_authorized=true`, retain the generated plan, then provision and prepare all four distinct runners |
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Gemma image](evidence/gate4-20260826-b-gemma-4-e2b-publication-evidence.json), and the same split-region fleet retains four distinct machine identities | Reuse the authorized exact fleet only after reviewing Gate 5, then retain all four Gemma profile results plus the aggregate |
