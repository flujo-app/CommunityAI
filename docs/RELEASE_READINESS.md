...
| 9 | Publish edge resource envelopes for selectable profiles | IN PROGRESS | `drift edge-benchmark` emits schema-v2 cleanup evidence and its focused tests pass. [Run `gate9-20260829-b`](evidence/gate9-20260829-b-edge-envelope-attempt.json) produced a passing real Windows Qwen envelope, then failed closed after real Linux Qwen inference retained 128.2 MB RSS above baseline after five seconds; Gemma did not start. The route's earlier Linux Torch shared-memory crash is permanently fixed at `4af96d1`; the benchmark's glibc arena trim at `a2df2b7` passed focused tests and independent review. Every exact B resource is absent, GPU usage is zero, and the protected bootstrap is running. Four passing Windows/Linux Qwen/Gemma envelopes are still required. | Run `gate9-20260829-c` is owner-authorized and reserved at USD 28 against pushed source `1e845e6`; execute one clean sequential attempt under the same one-attempt, five-minute no-progress, 60-minute model, and 90-minute deletion limits. |
| 10 | Implement automatic contributor model and block placement | IN PROGRESS | Signed bootstrap now installs one bounded `auto` worker. The local planner filters exact manifested candidates through owner policy and local resource ceilings, requires fresh authenticated replica coverage, targets the least-covered contiguous range with per-node jitter, reconciles exact-manifest launches through the existing artifact-verifying server and `WorkerSupervisor`, applies residency/cooldown/switch hysteresis, exposes placement reasons, and preserves an explicit operator pause across ineligibility or placement changes. The 145-test focused offline matrix, formatting checks, and independent public-API race probe pass; the probe proves an operator pause wins over a stale automatic replacement while ordinary automatic migrations still restart. | Add privacy-safe demand, useful-throughput/reliability weighting, and an expiring pre-download intent lease without weakening coverage or local-policy authority; later Gates 13–14 must prove the packaged flow and real hardware ceilings after Gate 9 envelopes exist. |
...
billing is delayed. The new `gate9-20260829-c` reservation consumes another USD 28 and
