...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | IN PROGRESS | The USD 69 `gate5-20260827-a` reservation is active; native auth, immutable images, one-L4 quota/usage, accelerator types, and protected-bootstrap inventory passed. The first network-only preflight was deleted and absence-proved. The replacement 29-test plan serializes four distinct hosts with exact per-profile cleanup. On the live Windows CPU host, the exact Qwen snapshot passed the eight-artifact preflight and the worker loaded/announced 24/24 blocks; the real client then exposed a missing runtime-cache argument and the Windows wheel's omitted dependency closure. Focused fixes pass 18 tests; no profile report exists yet. | Push the source fix, update the live host to that exact commit, rerun `windows-cpu`, retain its report, delete/absence-prove that host, then continue `linux-cpu`, `windows-cuda`, and `linux-cuda` one at a time. |
...
| gate5-20260827-a | GCP | Gate 5 Qwen3.5 2B Windows/Linux qualification; exact source will advance from the failed `55ddff6` attempt to its bounded runtime-cache fix | USD 69.00 | — | First perimeter deleted and absence-proved; serialized retry perimeter and Windows CPU phase are live under the 13.5-hour provider DELETE deadline. Final cleanup pending. | ACTIVE |
