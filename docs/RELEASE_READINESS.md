...
| 11 | Remove single-provider discovery and catalog availability | PAUSED | One GCP discovery peer is live; the node now has a bounded, exact-seed-scoped, atomic private cache of fresh Hivemind-valid global-IP TCP routing peers, merged into discovery, inference clients, contribution workers, and policy reloads without mutating persisted configuration; second-seed planning and discovery-container work remain preserved on `codex/fly-discovery-seed-adapter` | Do not extend Gate 11 until Gates 4–10 pass. Then provision the real second seed, publish two HTTPS catalog mirrors, and run seed-loss plus cached-peer recovery drills |
