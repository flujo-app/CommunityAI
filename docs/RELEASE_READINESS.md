...
outcome. The current owner-reset budget epoch retains Gate 6's USD 79 conservative
maximum, so only USD 21 remains until observed billing or another explicit owner reset.
...
| 7 | Qwen3.5 2B real separate-machine recovery | READY | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. USD 21 remains in the current accounting epoch; no Gate 7 spend or resource has been authorized yet. This is a Linux CPU recovery test, not CUDA qualification or a GPU benchmark. | Price one CPU-only bootstrap plus four CPU-only workers with immutable inputs and hard deletion deadlines. Create them only if the complete conservative maximum fits USD 21; otherwise record the budget blocker. Kill the selected worker during generation, prove exact recovery and cleanup, and never request or claim Fly GPU capacity. |
...
evidence but do not consume the new authorization epoch. The cleaned `gate6-20260827-a` run retains its USD 79 maximum while billing is delayed,
leaving **USD 21** for a fully priced later run. The additional USD 10 covered 48 GB rather
than 32 GB RAM on the two serial CUDA profiles after the empirical Windows failover-load
crash; their original absolute runtime backstops remained unchanged.
