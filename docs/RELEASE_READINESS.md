...
| ---: | --- | --- | --- | --- |
| 1 | Integrate the active revival branch and make its CI workflows dispatchable from the repository default branch | IN PROGRESS | [PR #8](https://github.com/flujo-app/CommunityAI/pull/8) carries the 17-commit branch; the exact offline selection passes locally (409 passed, 10 skipped), Black passes (258 files unchanged), and the corrected tracked-file isort check passes | Push the CI corrections, require Tests, Check style, and Windows/Linux Production desktop checks to pass on PR #8, then integrate it into `main` |
