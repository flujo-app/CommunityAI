...
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | IN PROGRESS | The serialized 13.5-hour G2/L4 fleet guard prices `gate6-20260827-a` at USD 69 maximum against exact source `a45025a3262a88df65217b630392488e8548aaaf`. Preflight found only the protected bootstrap running, global and `us-east1` L4 quota limit 1 with usage 0, L4 stock advertised in three fallback zones, and both pinned OS images READY. The immutable Gate 4 Gemma input remains exact. | Commit the reservation, rerun the guard until `provisioning_authorized` is true, create the private perimeter, then run Windows CPU/CUDA and Linux CPU/CUDA one host at a time with hard deletion deadlines and exact cleanup verification. |
...
| gate6-20260827-a | GCP | Gate 6 Gemma 4 E2B Windows/Linux CPU/CUDA qualification [source a45025a3262a88df65217b630392488e8548aaaf] | USD 69.00 | — | Not provisioned | PLANNED |
