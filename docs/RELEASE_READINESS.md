...
| 7 | Qwen3.5 2B real separate-machine recovery | READY | Provider-neutral controller and the 46-test Fly suite pass locally; provisioning now requires an exact Qwen publication report, derives its immutable runtime manifest, recomputes its rootfs requirement from measured uncompressed bytes, and sends that bounded size on all five Machine creates before selected-worker hard kill and fail-closed cleanup | After its accepted Windows/Linux matrix and Gate 4 report exist, reserve a priced Fly ledger row, run one bootstrap plus four workers, kill the selected worker during generation, prove exact recovery and cleanup |
| 8 | Gemma 4 E2B real separate-machine recovery | READY | The same evidence-bound adapter accepts the exact 35-block Gemma manifest and its larger reviewed publication/rootfs ceilings in deterministic tests | After Gate 6 and the Gemma Gate 4 report pass, repeat the exact priced Fly gate and retain bounded recovery and cleanup evidence |
...
| 11 | Remove single-provider discovery and catalog availability | IN PROGRESS | One GCP discovery peer is live; the node now has a bounded, exact-seed-scoped, atomic private cache of fresh Hivemind-valid global-IP TCP routing peers, merged into discovery, inference clients, contribution workers, and policy reloads without mutating persisted configuration; 94 focused discovery/node/worker/catalog-bootstrap tests pass locally; the schema-v2 cost guard binds every reservation to the canonical provider-plan digest and emits a finite-horizon Fly seed plan restricted to its run-derived app plus the expected reviewed-GHCR image/evidence identity, while type-safe canonical public bootstrap transport and workload isolation pass 113 focused cost/catalog tests | Build the immutable discovery-only image and a fail-closed Fly lifecycle adapter that semantically validates its publication evidence before provider authentication, then reserve and provision the priced second seed on a separate provider for the authorized horizon, publish two HTTPS catalog mirrors, and run real seed-loss plus cached-peer recovery drills; independent human operation remains a stable-release follow-up if unavailable for alpha |
