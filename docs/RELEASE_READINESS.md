...
| 4 | Build immutable Qwen3.5 2B and Gemma 4 E2B qualification images/snapshots | IN PROGRESS | A commit-derived tracked-only source context, exact candidate manifest/snapshot contract, and credential-free offline Dockerfile pass 25 deterministic tests; linked repository/snapshot roots fail before output materialization, source, Dockerfile, manifest, byte inventory, and artifact hashes are reverified inside the build, and no image is claimed built or published | On a Docker-enabled authenticated builder, materialize both unlinked exact snapshots, execute the emitted provenance/SBOM push plans, and record immutable OCI digests plus bounded compressed sizes; the current host has no Docker daemon or `gcloud` login |
