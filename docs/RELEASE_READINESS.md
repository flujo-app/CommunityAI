...
| Autonomous desktop formation | **OPEN; live Modal replay in progress after three cleaned GCP stockouts.** The [formation runners](QWEN_FORMATION_TEST.md) supply capacity only. Placement regressions and the local packaged-node/real-Qt preflight passed. Modal adds actual remote Qt Start sharing clicks; its TCP/cleanup probe passed and its first full attempt exposed a now-persisted image dependency fix. [Initial checkpoint](evidence/qwen-formation-checkpoint-20260907.json). No distributed formation pass is claimed. |
