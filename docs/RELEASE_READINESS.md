...
| 7 | Qwen3.5 2B real separate-machine recovery | IN PROGRESS | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. The [verified CPU-only publication](evidence/gate7-20260828-b-qwen3.5-2b-publication-evidence.json) binds source `7570d94`, exact Qwen artifacts, SLSA/SPDX attestations, immutable index `sha256:1969b62...`, and an 8 GB Fly rootfs; its builder and disk are absent. The [zero-resource Fly attempt](evidence/gate7-20260828-b-separate-machine-attempt.json) proved a private external image cannot be fetched. The [cleaned mirror attempt](evidence/g7mirror-20260828-c-fly-registry-attempt.json) exposed that this never-deployed app first required Fly's supported build-only repository initialization; that initialization pushed a zero-byte sentinel and created no Machine. This is Linux CPU recovery, not CUDA qualification or a GPU benchmark. | Complete one final bounded digest-preserving mirror, delete its builder and validate the immutable Fly-registry index, then create one CPU-only bootstrap plus four CPU-only 4-performance-vCPU/16 GB workers in `gru`. Kill the selected worker during generation, prove exact recovery and complete run-tag cleanup, and never request or claim Fly GPU capacity. |
...
| g7mirror-20260828-d | GCP | Final Gate 7 immutable Qwen mirror after supported Fly repository initialization; `e2-standard-2`, 30 GB disk, two-hour DELETE deadline, egress, contingency | USD 10.00 | — | Not provisioned | PLANNED |
...
created no Machine. Final retry `g7mirror-20260828-d` reserves USD 10 for the same bounded
mirror now that the repository exists. The four builder maxima plus the Fly reservation
leave **USD 30**. The Fly reservation prices five same-region CPU-only Machines at 4
performance vCPUs and 16 GB RAM each for a bounded run, with headroom for image startup,
cleanup, and minor network/rootfs charges. The final GCP reservation prices one two-hour
