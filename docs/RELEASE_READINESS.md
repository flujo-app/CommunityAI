...
real Gate 7 run. Its USD 30 conservative reservation leaves USD 70.
...
| 7 | Qwen3.5 2B real separate-machine recovery | IN PROGRESS | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. After Gate 6 cleanup, the owner explicitly reset the USD 100 epoch and authorized the real run. Native Fly authentication is valid, the isolated `petals-revival-smoke` app is empty, and the immutable Qwen qualification image remains the exact offline input. This is Linux CPU recovery, not CUDA qualification or a GPU benchmark. | Commit the USD 30 reservation, re-prove the app empty, then create one CPU-only bootstrap plus four CPU-only 4-performance-vCPU/16 GB workers in `gru`. Kill the selected worker during generation, prove exact recovery and complete run-tag cleanup, and never request or claim Fly GPU capacity. |
...
`gate7-20260827-a` reservation commits USD 30 maximum and leaves **USD 70**. It prices
five same-region CPU-only Fly Machines at 4 performance vCPUs and 16 GB RAM each for
a bounded run, with headroom for image startup, cleanup, and minor network/rootfs charges.
Fly credit is not counted as extra authorization.
