...
| 11 | Remove single-provider discovery and catalog availability | PAUSED | One GCP discovery peer is live; peer caching and bounded second-seed planning are partially implemented, and an unfinished discovery-container slice is preserved on `codex/fly-discovery-seed-adapter` | Do not extend Gate 11 until Gates 4–10 pass. Preserve the existing branch, then resume with the real second seed, two mirrors, and seed-loss/cached-peer drills |
