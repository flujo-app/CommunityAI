...
| 14 | Pass contribution-control hardware checks | IN PROGRESS | The authenticated desktop contract now preserves model, schedule, storage, VRAM, bandwidth, power, and telemetry admission; the Sharing page renders resolved limits and unavailable-provider reasons, starts only admitted workers, keeps pause available for selected workers, and no longer stores an unenforced local VRAM preference; 22 focused tests, the 458-pass/8-skip offline CI selection, Black and isort across 262 Python files, and source UI/self-tests pass | Add atomic validated persistence for editing every policy value from the GUI, then validate enforcement, suspension, pause timing, and unsupported telemetry on real packaged Windows/Linux hardware; this repository slice is not hardware evidence |
