...
| 9 | Publish edge resource envelopes for selectable profiles | IN PROGRESS | `drift edge-benchmark` now emits schema-v2 post-close runtime, route-manager, process-tree RSS/child-process, accelerator, and bounded-stabilization evidence, releases local tensors before its final sample, and exits unsuccessfully when cleanup is not proved. Cleanup compares child-process identity, permits only 16 MiB of RSS allocator jitter, and waits at most five seconds for asynchronous teardown. Retained-RSS and replacement-child regressions plus the focused Gate 9, manifest, loading-diagnostics, route-health, model-manager, and node suite pass 68 tests with one platform skip; the complete declared Windows/Linux offline suite passes 519 tests with 11 skips. Refreshed real Qwen/Gemma Windows/Linux envelopes are still absent. | Push the verified reporting slice, then execute Qwen and Gemma sequentially on Windows and Linux under the [Gate 9 runbook](EDGE_RESOURCE_ENVELOPE_RUNBOOK.md) one-attempt, five-minute no-progress, 60-minute model, and 90-minute deletion limits. |
...
| --- | --- | --- | ---: | ---: | --- | --- |
| gate9-20260829-a | GCP | Gate 9 sequential Qwen/Gemma edge envelopes: one G2/L4 route and one native Linux client per model, Windows client local, 60-minute model limits, 90-minute DELETE backstops, disks, egress, and contingency | USD 28.00 | — | Reservation recorded before create; exact run-ID resources must be absent and protected bootstrap must remain running. | RESERVED |
