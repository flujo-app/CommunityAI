...
| 10 | Implement automatic contributor model and block placement | IN PROGRESS | Signed bootstrap now installs one bounded `auto` worker. The local planner filters exact manifested candidates through owner policy and local resource ceilings, requires fresh authenticated replica coverage, targets the least-covered contiguous range with per-node jitter, reconciles exact-manifest launches through the existing artifact-verifying server and `WorkerSupervisor`, applies residency/cooldown/switch hysteresis, exposes placement reasons, and preserves an explicit operator pause across ineligibility or placement changes. A new or migrated worker must now sign an expiring exact-manifest/range intent with fixed numeric resource claims and receive a remote DHT store acknowledgement (`exclude_self=True`) before entering the artifact path; invalid, rejected, or failed publication is fail-closed and cannot advance planner state, while a previously admitted placement is retained. The 98-test focused and 167-test broader offline matrices, formatting/import checks, public-API pause-race probe, and direct publication-failure probes pass. | Add coarse privacy-safe demand plus completed-route useful-throughput/reliability weighting without weakening signed intent, coverage, or local-policy authority. Durable cross-restart replay history, Sybil-resistant demand aggregation, and convergence/load testing remain mandatory Gate 10 work; later Gates 13–14 must prove the packaged flow and real hardware ceilings after Gate 9 envelopes exist. |
