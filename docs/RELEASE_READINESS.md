...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Qwen image](evidence/gate4-20260826-b-qwen3.5-2b-publication-evidence.json). Provider preflight found one T4 slot in each of `us-central1` and `us-east1`; the 25-test [authorized plan](evidence/qual-20260826-b-cost-plan.json) binds exact OS images and safely assigns only Linux CUDA to the fallback region without changing the USD 69 maximum. | Execute the authorized plan in order, verify exact boot sources and host prerequisites, then register and run all four distinct profiles |
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | READY | The strict four-profile matrix can use the passed [immutable Gate 4 Gemma image](evidence/gate4-20260826-b-gemma-4-e2b-publication-evidence.json), and the same [split-region fleet](evidence/qual-20260826-b-cost-plan.json) retains four distinct machine identities | Reuse the authorized exact fleet only after reviewing Gate 5, then retain all four Gemma profile results plus the aggregate |
...
| qual-20260826-b | GCP | Four-host Windows/Linux qualification fleet [source 7660e33e03326e5b868f81cb95282460ba649d5f] | USD 69.00 | — | Not provisioned | PLANNED |
