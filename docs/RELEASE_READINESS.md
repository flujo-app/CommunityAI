...
| Autonomous desktop formation | **OPEN; corrected GCP N2 replay in progress.** The one-click replay passed six local-answer/UI checks, reached 32/64 automatically placed blocks, and confirmed the slow-growth placement fix after 15 minutes. It then exposed an unbounded discovery startup and a null-coverage runner error; both are fixed in `17ffeb2`, with 72 related tests passed. [Evidence and cleanup](evidence/qwen-formation-discovery-startup-20260907.json). The [runners](QWEN_FORMATION_TEST.md) supply capacity only. Full formation/promotion/recovery has not passed. |
