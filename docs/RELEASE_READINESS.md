...
| 9 | Publish edge resource envelopes for selectable profiles | IN PROGRESS | `drift edge-benchmark` emits schema-v2 cleanup evidence and its focused tests pass. [Run `gate9-20260829-a`](evidence/gate9-20260829-a-edge-envelope-attempt.json) stopped before producing an envelope when overlapping orchestration launched two Windows Qwen processes; both processes and every exact GCP resource are absent, GPU usage is zero, and the protected bootstrap is running. Refreshed real Qwen/Gemma Windows/Linux envelopes are still absent. | Execute owner-authorized clean run `gate9-20260829-b` through one controller, sequentially, under the one-attempt, five-minute no-progress, 60-minute model, and 90-minute deletion limits. |
...
| --- | --- | --- | ---: | ---: | --- | --- |
| gate9-20260829-b | GCP | Owner-authorized clean Gate 9 retry through one controller: sequential Qwen/Gemma routes and Windows/Linux cold clients, 60-minute model limits, 90-minute DELETE backstops, disks, egress, and contingency | USD 28.00 | — | Reservation recorded before create in the owner-reset epoch; exact run-ID resources must be absent and protected bootstrap must remain running. | RESERVED |
