...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | IN PROGRESS | The USD 69 `gate5-20260827-a` reservation is recorded for exact source `55ddff6`; native auth, immutable images, one-L4 quota/usage, accelerator types, and protected-bootstrap inventory passed. The first network-only preflight exposed an unsafe flat two-CUDA plan, unnameable NAT addresses, and missing bare-image SSH/driver bootstrap before any VM existed; its complete perimeter was deleted and absence-proved. The replacement 29-test plan serializes four distinct hosts with per-profile cleanup, named NAT addresses, Windows SSH, and checksum-pinned CUDA bootstrap. No profile report exists yet. | After independent verification, regenerate the authorized phase plan, execute `windows-cpu`, `linux-cpu`, `windows-cuda`, and `linux-cuda` one host at a time, retain the exact aggregate, and prove complete cleanup. |
...
| gate5-20260827-a | GCP | Gate 5 Qwen3.5 2B Windows/Linux qualification [source 55ddff6cc6fa8c8ab8da399112ce9d5da3f0ff32] | USD 69.00 | — | First network-only preflight perimeter deleted and absence-proved; no VM created; phased retry not provisioned. | PLANNED |
