...
| 7 | Qwen3.5 2B real separate-machine recovery | IN PROGRESS | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. After Gate 6 cleanup, the owner explicitly reset the USD 100 epoch and authorized the real run. Native Fly authentication is valid and the isolated `petals-revival-smoke` app is empty. Two exact local Buildx publication attempts were terminated by the registry after 3,601.7 seconds before a large layer could finish; no Machine was created. This is Linux CPU recovery, not CUDA qualification or a GPU benchmark. | Use the reserved short-lived GCP CPU builder to reproduce and publish the exact source-bound image within the registry timeout, delete it, validate the immutable digest, then create one CPU-only bootstrap plus four CPU-only 4-performance-vCPU/16 GB workers in `gru`. Kill the selected worker during generation, prove exact recovery and complete run-tag cleanup, and never request or claim Fly GPU capacity. |
...
| gate7pub-20260827-a | GCP | Gate 7 exact Qwen CPU image publisher after repeat 3,601.7-second Fly registry disconnects; 80 GB disk, four-hour DELETE deadline, egress, and contingency | USD 10.00 | — | Not provisioned | PLANNED |
...
`gate7pub-20260827-a` recovery reservation commits USD 10 for one short-lived
CPU-only GCP builder after two exact local pushes hit the registry's 3,601.7-second
connection limit. Together they leave **USD 60**. The Fly reservation prices five
