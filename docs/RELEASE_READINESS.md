...
| 5 | Qwen3.5 2B Windows/Linux CPU/CUDA qualification | IN PROGRESS | The first [authorized fleet capacity attempt](evidence/qual-20260826-b-capacity-attempt-1.json) stopped when `us-central1-a` returned `ZONE_RESOURCE_POOL_EXHAUSTED` for Windows T4, then fully cleaned every exact run resource while retaining the protected bootstrap. The same USD 69 reservation now has a [provider-authorized attempt-2 plan](evidence/qual-20260826-b-cost-plan-attempt-2.json) for preflighted `us-east1-c`/`us-west1-b`, exact images, and CUDA-first creation. | Execute attempt 2 in order, verify exact boot sources and host prerequisites, then register and run all four distinct Qwen profiles against the immutable Gate 4 snapshot |
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | WAITING | The strict four-profile matrix can use the passed [immutable Gate 4 Gemma image](evidence/gate4-20260826-b-gemma-4-e2b-publication-evidence.json), but the shared fleet's first [capacity attempt](evidence/qual-20260826-b-capacity-attempt-1.json) was cleaned before any qualification job and Gate 5 has not passed | Reuse the exact replacement fleet only after reviewing Gate 5, then retain all four Gemma profile results plus the aggregate |
...
| qual-20260826-b | GCP | Four-host Windows/Linux qualification fleet [source 7660e33e03326e5b868f81cb95282460ba649d5f] | USD 69.00 | — | [Attempt 1](evidence/qual-20260826-b-capacity-attempt-1.json) fully cleaned; replacement not provisioned | PLANNED |
