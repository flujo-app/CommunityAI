...
The current mandatory sequence is **Gates 7–8 → Gates 9–16 → Gate 17**. The visible
...
real Gate 7 run. Its USD 30 Fly reservation plus four bounded USD 10 GCP publication and mirror reservations leave USD 30.
...
| 6 | Gemma 4 E2B Windows/Linux CPU/CUDA qualification | PASSED | [Qualification and cleanup evidence](evidence/gate6-20260827-gemma-4-e2b-qualification.json) and the [strict aggregate](evidence/gate6-20260827-gemma-4-e2b-matrix.json) bind Windows CPU/CUDA and Linux CPU/CUDA passes to exact source `a45025a3262a88df65217b630392488e8548aaaf`, DRIFT `2.3.0.dev2`, Gemma revision `3e22461f65e89153144f8adb70e3b8c2cc9845a7`, and manifest `sha256:2f8debbe0fcdf5af8d4c56c982210fa50aa584314968ae2617e2ccc2de9eafdd`. Every profile proved exact artifacts, 35/35 manifested stock-token parity, selected-worker interruption, and recovery. All Gate 6 instances, disks, firewall, NATs, routers, subnets, addresses, and VPC are absent; global GPU and regional L4 usage are zero; the protected bootstrap remains running. | Use the recorded Gate 7 Fly reservation only after retry-d cleanup is proved and the immutable mirror passes. |
| 7 | Qwen3.5 2B real separate-machine recovery | BLOCKED | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. The [verified CPU-only publication](evidence/gate7-20260828-b-qwen3.5-2b-publication-evidence.json) binds source `7570d94`, exact Qwen artifacts, SLSA/SPDX attestations, immutable index `sha256:1969b62...`, and an 8 GB Fly rootfs. The [cleaned mirror attempt](evidence/g7mirror-20260828-c-fly-registry-attempt.json) initialized the isolated Fly repository without creating a Machine. [Retry d](evidence/g7mirror-20260828-d-fly-registry-attempt.json) then created the bounded GCP mirror builder but failed before copying because a stale GHCR credential shadowed valid anonymous source access. Fly has zero active Machines and app tokens; GCP builder/disk cleanup and protected-bootstrap status are unproved because exact-name read-only queries still required interactive `gcloud` reauthentication at `2026-08-28T20:29:14Z`. This is Linux CPU recovery, not CUDA qualification or a GPU benchmark. | Owner reauthenticates native `gcloud`; then verify or exact-delete only the retry-d builder/disk and prove the protected bootstrap is running. Reserve a new bounded USD 10 retry only after cleanup proof, mirror with anonymous source plus destination-only authentication, validate the immutable Fly index, and run the five CPU-only `gru` Machines with selected-worker interruption and complete cleanup. Never request or claim Fly GPU capacity. |
...
| g7mirror-20260828-d | GCP | Final Gate 7 immutable Qwen mirror after supported Fly repository initialization; `e2-standard-2`, 30 GB disk, intended two-hour DELETE deadline, egress, contingency | USD 10.00 | — | [Failed attempt and partial cleanup evidence](evidence/g7mirror-20260828-d-fly-registry-attempt.json): builder was created; copy did not start because GHCR authentication was rejected; Fly has zero Machines/tokens; GCP instance/disk absence and protected-bootstrap state await native `gcloud` reauthentication. | BLOCKED |
...
Its GCP cleanup is unproved until native `gcloud` is reauthenticated. The four builder
maxima plus the Fly reservation leave **USD 30**; a newly reserved USD 10 retry would
leave USD 20. The Fly reservation prices five same-region CPU-only Machines at 4
performance vCPUs and 16 GB RAM each for a bounded run, with headroom for image startup,
cleanup, and minor network/rootfs charges. Retry d's GCP maximum priced one intended two-hour `e2-standard-2` CPU mirror, its
30 GB balanced disk, network egress, headroom, and contingency. No new provider resource
may be created until retry d cleanup is live-proved. Fly credit is not counted as extra authorization.
