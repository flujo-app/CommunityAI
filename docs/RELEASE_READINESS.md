...
| ---: | --- | --- | --- | --- |
| 1 | Integrate the active revival branch and make its CI workflows dispatchable from the repository default branch | IN PROGRESS | Local branch contains the current qualification/catalog/contribution work; the CI-listed offline selection passes locally (409 passed, 10 skipped) | Push through the normal branch/PR path and require the repository checks to pass before default-branch integration |
