...
| 7 | Qwen3.5 2B real separate-machine recovery | WAITING | Gate 4 passed and the provider-neutral controller plus evidence-bound Fly adapter pass; provisioning requires the exact Qwen publication report, derives its immutable runtime manifest, recomputes rootfs from measured uncompressed bytes, and applies that bounded size to all five Machines, but the Gate 5 four-profile matrix has not passed | After Gate 5 passes, reserve the priced Fly run, create one bootstrap plus four workers, kill the selected worker during generation, and prove exact recovery plus complete cleanup |
| 8 | Gemma 4 E2B real separate-machine recovery | WAITING | Gate 4 passed and the same evidence-bound adapter accepts the exact 35-block Gemma manifest with its reviewed publication and rootfs ceilings, but the Gate 6 four-profile matrix has not passed | After Gate 6 passes, repeat the real priced Fly interruption gate and retain recovery plus cleanup evidence |
...
| 11 | Remove single-provider discovery and catalog availability | PAUSED | One GCP discovery peer is live; the node now has a bounded, exact-seed-scoped, atomic private cache of fresh Hivemind-valid global-IP TCP routing peers, and the schema-v2 cost guard binds reservations to a canonical provider-plan digest with a finite-horizon Fly seed plan restricted to its run-derived app and reviewed image/evidence identity; the real second seed and mirrors are not deployed | Do not extend Gate 11 until Gates 4–10 pass. Then build the immutable discovery image and fail-closed Fly lifecycle adapter, provision the priced second seed, publish two HTTPS catalog mirrors, and run seed-loss plus cached-peer recovery drills |
