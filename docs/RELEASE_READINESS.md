...
real Gate 7 run. Its USD 30 Fly reservation plus five bounded USD 10 GCP publication and mirror reservations leave USD 20.
...
| 7 | Qwen3.5 2B real separate-machine recovery | IN PROGRESS | Gate 5, Gate 6, and the controller/CPU-only Fly adapter pass. The [verified CPU-only publication](evidence/gate7-20260828-b-qwen3.5-2b-publication-evidence.json) binds source `7570d94`, exact Qwen artifacts, SLSA/SPDX attestations, immutable index `sha256:1969b62...`, and an 8 GB Fly rootfs. The [cleaned mirror attempt](evidence/g7mirror-20260828-c-fly-registry-attempt.json) initialized the isolated Fly repository without creating a Machine. [Retry d](evidence/g7mirror-20260828-d-fly-registry-attempt.json) failed before copying because a stale GHCR credential shadowed valid anonymous source access; its exact GCP builder/disk are now proved absent and the protected bootstrap is running. Fly has zero active Machines and app tokens. Retry e is bounded at USD 10 and the immutable source passed a fresh anonymous preflight. This is Linux CPU recovery, not CUDA qualification or a GPU benchmark. | Mirror with anonymous source plus destination-only authentication, validate the immutable Fly index, and run the five CPU-only `gru` Machines with selected-worker interruption and complete cleanup. Never request or claim Fly GPU capacity. |
...
| g7mirror-20260828-e | GCP | Gate 7 immutable Qwen mirror with anonymous source and destination-only Fly authentication; `e2-standard-2`, 30 GB balanced disk, two-hour DELETE deadline, egress, and contingency | USD 10.00 | — | Not provisioned | PLANNED |
...
running. Retry `g7mirror-20260828-e` reserves USD 10 for the anonymous-source,
destination-only mirror. The five builder maxima plus the Fly reservation leave
**USD 20**. The Fly reservation prices five same-region CPU-only Machines at 4
performance vCPUs and 16 GB RAM each for a bounded run, with headroom for image startup,
cleanup, and minor network/rootfs charges. Retry e's GCP maximum prices one two-hour
`e2-standard-2` CPU mirror, its 30 GB balanced disk, network egress, headroom, and
contingency. Fly credit is not counted as extra authorization.
