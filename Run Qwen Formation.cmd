...
if exist ".gate13-runs\qwen-product-venv\Scripts\python.exe" set "QWEN_PYTHON=.gate13-runs\qwen-product-venv\Scripts\python.exe"
if not defined QWEN_PYTHON if exist ".venv-cuda\Scripts\python.exe" set "QWEN_PYTHON=.venv-cuda\Scripts\python.exe"
...
if not defined QWEN_PYTHON (
  ...
  call "%QWEN_PYTHON%" %QWEN_PYTHON_ARGS% scripts\run_qwen_formation.py %*
  set "QWEN_EXIT=!ERRORLEVEL!"
)
echo.
echo Qwen formation finished with exit code !QWEN_EXIT!.
