...
class LaunchNodeTests(TestCase):
    def test_frozen_node_dispatches_edge_acquisition_without_source_entrypoint(self):
        with (
            patch.object(
                sys,
                "argv",
                ["CommunityAI-Node", "edge-acquire", "manifest.json", "--cache_dir", "cache"],
            ),
    ...
    def test_frozen_node_exposes_distinct_worker_self_test(self):
        expected = {
            "schema_version": 1,
    ...
    def test_worker_self_test_exercises_bounded_worker_entrypoint_without_model_or_network(self):
        with patch(
    ...
    def test_worker_self_test_fails_when_process_lifetime_guard_is_unavailable(self):
        with (
            patch(
                "drift.utils.process_lifetime.tie_child_processes_to_this_process",
                return_value=False,
            ),
            self.assertRaisesRegex(RuntimeError, "process-lifetime guard"),
        ):
