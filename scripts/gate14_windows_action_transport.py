...
_ACTION_HOST_SHA256 = "fbfcabf3842ace1ccb9dac41ee702fbc9b65206a29f3a76ca820bc1ef2906812"
...
class WindowsActionTransport:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            or not 0.1 <= float(response_timeout_seconds) <= 300.0
            or type(close_timeout_seconds) not in (int, float)
            or not 0.1 <= float(close_timeout_seconds) <= 60.0
        ):
    ...
    def request(self, operation: str, payload: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        try:
            response_item = self._responses.get(timeout=self._response_timeout)
        except queue.Empty:
